    ...
    <Link to={`/blogs/${id}`}>
      <div className=" cursor-pointer">
        <div className="pl-20 pt-10 max-w-screen-md ">
          ...
          <div className="font-bold text-2xl pt-1">{title}</div>
          <div className="pt-1">{content.slice(0, 100) + "..."}</div>
...
export function Avatar({ name }: { name: string; size?: number }) {
  return (
    <div
      className={`relative inline-flex items-center justify-center w-7 h-7 mr-2 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600`}
    >
